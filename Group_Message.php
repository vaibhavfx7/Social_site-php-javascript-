<?php
	...
	if(isset($_SESSION['fbadmin']))
	{
		...
		if(isset($_POST['delete_chat']))
		{
			...
			mysqli_query("delete from group_chat where chat_id=$chat_id");
		}
...
<?php
		$que_chat=mysqli_query("select * from group_chat order by chat_id desc");
?>
...
<?php
	while($chat_data=mysqli_fetch_array($que_chat))
	{
		...
		$que_fb_user_info=mysqli_query("select * from users where user_id=$fb_user_id");
		...
		$que_fb_user_pic=mysqli_query("select * from user_profile_pic where user_id=$fb_user_id");
